const ButtonContainer = ({classNames, reviewer}) => {
	const prevReviewer = () => {
		if (reviewer.number == 0) 
			reviewer.setNumber(5);
		else reviewer.setNumber(reviewer.number-1);
	};
	const nextReviewer = () => {
		if (reviewer.number == 5) 
			reviewer.setNumber(0);		
		else reviewer.setNumber(reviewer.number+1);
	};	
	return (
		<div className={classNames.buttonContainer}>
			<button className={classNames.prevBtn} onClick={prevReviewer}>
			...
			<button className={classNames.nextBtn} onClick={nextReviewer}>
